package project.instagram.common.enums.constants;

public class HashtagConstants {
	
	public static final String CREATED_HASHTAG_SUCCESSFULLY = "Created hashtag successfully";
	
	public static final String HASHTAG_EXISTS = "Hashtag has been existed!";
	
	public static final String HASHTAG_IN_BLACK_LIST_HASHTAGS = "Hashtag is in black list hashtags";
	
	public static final String HASHTAG_NOT_EXISTS = "Hashtag hasn't existed";
	
	public static final String UPDATED_HASHTAG_SUCCESSFULLY = "Updated hashtag successfully";
	
	public static final String DELETED_HASHTAG_SUCCESSFULLY = "Deleted hashtag successfully";
	
	public static final String DELETED_HASHTAG_FAILED = "Deleted hashtag failed";
	
	public static final String ENABLED_CRAWL_HASHTAG_FAILED = "Enabled crawl hashtag failed";
	
	public static final String ENABLED_CRAWL_HASHTAG_SUCCESSFULLY ="Enabled cralw hashtag successfully";
}
